...
print(f"Found {len(series_urls)} Series URLs and {len(watch_urls)} Watch URLs in sitemap.")
...
def get_episode_url(base_slug, ep_num):
    ...
    # Check exact ep mapping in sitemap
    target_pattern = f"{clean_slug}-ep-{ep_num}.html"
    for url in watch_urls:
        if target_pattern in url:
            return url
            
    # Try another pattern like with missing dashes
    # Just in case, return a constructed URL
    return f"https://nordrama.live/watch/{clean_slug}-ep-{ep_num}.html"

# Fix links inside Ramadan directory
if os.path.exists(RAMADAN_DIR):
    ...
    for filename in os.listdir(RAMADAN_DIR):
        if filename.endswith('.html'):
            ...
            # Find the main CTA buttons and update their direct href.
            # We target the most common occurrences: current tomito.xyz/tv/ or similar or ramadan-trailer links
            new_content = re.sub(r'href="(?:https://tomito\.xyz/)?(?:ramadan-trailer/|tv/|movie/|watch/|series/)[^"]*"', f'href="{base_series_url}"', content, count=0)
