...
def update_html_posters(d, filename, poster):
    ...
    # Find the image inside <div class="series-hero"> and replace only the src attribute
    new_content = re.sub(
        r'(<div class="series-hero">\s*<img\s+[^>]*?src=")[^"]+(")',
        r'\g<1>' + poster + r'\2',
        new_content,
        flags=re.IGNORECASE
    )
...
def main():
    dirs = ['/home/tomito/tomito/ramadan-trailer', '/home/tomito/tomito/series', '/home/tomito/tomito/watch', '/home/tomito/tomito/movies']
    
    # In-memory dictionary to save repeated API calls in the same run, but it completely ignores poster_cache.json
    cache = {}
    ...
    for d in dirs:
        ...
        for filename in os.listdir(d):
            if filename.endswith('.html'):
                ...
                if not poster:
                    print(f"Could not find poster for {filename}")
                    continue
    ...
    print(f"Complete! Updated images for {updated_count} HTML files using fresh TMDB data.")
